...
import { BehaviorSubject, Subject, catchError, combineLatest, map, shareReplay, switchMap, tap, throwError } from 'rxjs';
...
export class PostAlternateService {
  ...
  private postListParams: CombinedPostListParams = { _start: 0, _limit: 5, user: '', _search: '' }
  private postFilterParams$ = new BehaviorSubject<CombinedPostListParams>(this.postListParams);
  ...
  postWithFilters$ = this.postFilterActions$.pipe(
    ...
    switchMap( (filters: CombinedPostListParams) => {
      ...
      const modifiedFilters: CombinedPostListParams = savedFilters ? { ...filters, ...savedFilters } : filters;
      console.log(savedFilters)
      this.saveFilters(filters)
      return combineLatest([
        this.getPostList(modifiedFilters),
        this.userList$
      ]).pipe(
        map( ([posts, users ]) => {
